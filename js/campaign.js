...
function openFactionModal() {
  ...
  const teamsHTML = originalTeams.map(origCode => {
    const code = (typeof getEffectiveTeamCode === "function") ? getEffectiveTeamCode(origCode) : origCode;
    const team = (typeof getEffectiveTeamMetadata === "function") ? getEffectiveTeamMetadata(code) : (teamMetadata[code] || {});
    ...
    return `
      <div 
        ...
        class="p-2.5 bg-console-panel border ${isSelected ? 'border-amber-400 bg-amber-500/10 shadow-[0_0_15px_rgba(245,158,11,0.3)]' : 'border-console-border hover:border-amber-500/50'} rounded-lg cursor-pointer transition flex items-center space-x-2 group relative"
      >
        ...
        <button 
          onclick="event.stopPropagation(); closeFactionModal(); openSwapModal('${origCode}')" 
          title="${isZh ? '替换参战国' : 'Swap Qualified Nation'}" 
          class="text-[9px] px-1 py-0.5 bg-console-bg border border-console-border hover:border-amber-400 text-console-dimText hover:text-amber-400 rounded transition cursor-pointer shrink-0"
        >↻</button>
...
function updateCampaignHUD() {
  ...
  const effectiveTeams = originalTeams.map(t => (typeof getEffectiveTeamCode === "function") ? getEffectiveTeamCode(t) : t);
  if (!activeCampaignTeam || !effectiveTeams.includes(activeCampaignTeam)) {
    ...
    if (activeCampaignTeam && !effectiveTeams.includes(activeCampaignTeam) && typeof triggerAlert === "function") {
